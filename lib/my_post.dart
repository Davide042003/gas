...
class _MyPostState extends ConsumerState<MyPost>
  ...
  Future<void> fetchAnswersCounts(PostService postService, String postId, String idUserPost, int maxInnerListIndex) async {
    final answersCountsMap = await postService.getAnswersByIndex(postId, idUserPost);
  ...
  Future<void> fetchData() async {
    answersMap = await postService.getAnswersByIndex(widget.post.postId!, widget.user.id!);

    userFutures = answersMap[0]?.map((answer) => ref.read(otherUserProfileProvider(answer.id!).future))?.toList() ?? [];

    // Wait for all user profile futures to complete
    await Future.wait(userFutures);

    // Calculate total answers
    totalAnswers = answersMap.values.fold<int>(0, (sum, answers) => sum + answers.length);

    // Calculate answers count as a list of percentages
    answersCount = answersMap.values.map<double>((answers) => answers.length / totalAnswers).toList();
  }
      ...
      builder: (BuildContext context) {
        return CupertinoAlertDialog(
          ...
          actions: [
            ...
            CupertinoDialogAction(
              child: Text('Elimina', style: TextStyle(color: AppColors.backgroundRed)),
              onPressed: ()  {
              },
            ),
          ],
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    widget.post.images!.length == 2
                                      ...
                                      children: [
                                        ImageAnswer(
                                          answersCount: answersCount[0] ?? 0.0,
                                          imageUrl:
                                        ...
                                        ImageAnswer(
                                          answersCount:answersCount[1] ?? 0.0,
                                          imageUrl:
                                      ...
                                      children: [
                                        ...
                                        Row(
                                          ...
                                          children: [
                                            TextAnswer(
                                              answersCount:answersCount[2] ?? 0.0,
                                              answerText: widget.post
                                                  .answersList?[
                                              2] ??
                                                  "",
                                    ...
                                    Expanded(child: ListView.builder(
                                      ...
                                      itemCount: answersMap[0]?.length ?? 0,
                                      itemBuilder: (context, index) {
                                        final answer = answersMap[0]![index];

                                        return FutureBuilder<UserModel?>(
                                          future: userFutures[index],
                                          builder: (context, snapshot) {
                                            if (snapshot.connectionState == ConnectionState.waiting) {
                                              ...
                                              return GestureDetector(onTap:() {
                                                answer.isAnonymous! ? null : BottomSheetProfile.showOtherProfileBottomSheet(context, answer.id!);
                                              }, child: Container(
                                                height: 60,
                                    ...
                                    PopupMenuButton<String>(
                                      itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                                        PopupMenuItem<String>(
                                          value: 'delete',
                                          child: Text('Elimina domanda', style: TextStyle(fontFamily: 'Helvetica', fontSize: 16, color: AppColors.backgroundRed, fontWeight: FontWeight.bold)),
                                        ),
                                      ],
